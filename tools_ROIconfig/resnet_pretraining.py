...
def save_checkpoint(model, class_mapping, arch, store_dir="../checkpoint"):
    ...
    torch.save(checkpoint, f"{store_dir}/{timestamp}_{arch}.pth")
...
def main():
    ...
    if freeze:
        ...
        raise NotImplementedError("Fine-tuning without freezing not implemented")
    ...
    cache_dir = "resnet_cache"
    if not os.path.exists(cache_dir):
        os.makedirs(cache_dir)

    # Train model
    resnet_model, plot_train, plot_val = train_model(model, optimizer, criterion, train_loader, val_loader, epochs=epochs)
