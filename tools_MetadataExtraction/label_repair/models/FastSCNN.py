...
class FastSCNN(nn.Module):
    def __init__(self, in_channels, num_classes, task="classification"):
        super().__init__()
        ...
        if task == "classification":
            self.classifier = Classifier(num_classes, scale_factor=16)
        elif task == "regression":
            self.classifier = Regressor(scale_factor=8)
        self.task = task

    def forward(self, x):

        # perform upstacking in case of single images - strange bug
        upstacked = False
        if x.shape[0] == 1:
            x = torch.cat((x, x), dim=0)
            upstacked = True

        # perform reshaping - only certain sizes are allowed
        input_size = (x.shape[2], x.shape[3])
        n = 64
        if input_size[0]%n != 0 or input_size[1]%n != 0:
            target_size = (math.floor(input_size[0]/n) *n , math.floor(input_size[1]/n) * n )
            x = F.interpolate(x.float(), size=target_size, mode='bilinear', align_corners=False)
        else:
            target_size = None
        ...
        # undo reshaping / reshape back to original size
        #print(x.shape)
        input_size = (input_size[0] * 2, input_size[1] *2 )
        #print(input_size)
        output_size = (600,788)
        if target_size is not None:
            x = F.interpolate(x.float(), size=output_size, mode='bilinear', align_corners=False)

        # undo upstacking to deliver a single image
        if upstacked:
            x = x[0, :, :, :]
...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.drop_out(x)
        x = self.conv(x)
        x = F.interpolate(input=x, scale_factor=self.scale_factor,
...
class Regressor(nn.Module):
    def __init__(self, scale_factor):
        ...
        self.dsconv2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True))
        self.drop_out = nn.Dropout(p=0.1)
        self.conv = nn.Conv2d(128, 1, kernel_size=1, stride=1, padding=0, bias=True)
...
if __name__ == '__main__':
    ...
    #%%
    img = torch.randn(1, 3, 600, 789)
    model = FastSCNN(in_channels=3, num_classes=2)
    outputs = model(img)
    print(outputs.shape)

    model = FastSCNN(in_channels=3, num_classes=2, task="regression")
    outputs = model(img)
    print(outputs.shape)